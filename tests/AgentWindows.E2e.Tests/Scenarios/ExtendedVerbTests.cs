...
public sealed class ExtendedVerbTests(TargetAppFixture fixture) : IClassFixture<TargetAppFixture>
{
    ...
    public async Task Window_ResizeAndMove_UpdateTheReportedBounds()
    {
        ...
        var resized = resize.ShouldSucceedWith<Core.Protocol.Windows.WindowPayload>().Window;
        ...
        var moved = move.ShouldSucceedWith<Core.Protocol.Windows.WindowPayload>().Window;
    ...
    public async Task Screenshot_Element_WritesAPngFile()
    {
        ...
        try
        {
            ...
            result.ShouldSucceedWith<Core.Protocol.Capture.ScreenshotPayload>().Path.ShouldBe(path);
